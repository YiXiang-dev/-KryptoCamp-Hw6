...
const MainMint = ({ accounts, setAccounts }) => {
  ...
  const getNFTTotalSupply = async () => {
    ...
    const KryptoCampContract = new ethers.Contract(KryptoCampNFTAddress, kryptoCampNFTAbi, provider)
    let totalSupply = await KryptoCampContract.totalSupply()
  ...
  const handleMint = async () => {
    if (window.ethereum) {
      ...
      const KryptoCampContract = new ethers.Contract(KryptoCampNFTAddress, kryptoCampNFTAbi, signer)
      try {
        // TODO: 4) 呼叫合約 mint 方法
        const MintPrice = CalMintPrice(mintAmount)    //計算mint價錢
        const mint  = await KryptoCampContract.mint(mintAmount, {value : MintPrice})
        console.log(mint)
      } catch ({ error }) {
  ...
  //計算mint價錢
  const CalMintPrice = (mintAmount) => {
    const price = ethers.utils.parseEther(mintprice).mul(mintAmount)
    return price
  }
